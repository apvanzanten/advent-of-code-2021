...
struct Board {
  ...
  constexpr bool mark_if_present(int n) {
...
int main() {
  ...
  for (int drawn_n : input.drawn_numbers) {
    std::cout << "drawing: " << drawn_n << std::endl;
    for (Board &board : input.boards) {
      const bool is_board_marked = board.mark_if_present(drawn_n);
      ...
      if (is_board_marked && board.has_full_column_or_row()) {
        ...
        const auto sum_of_unmarked =
            std::accumulate(board.numbers.begin(), board.numbers.end(), 0, [](auto s, auto n) {
              return s + (!n.is_marked ? n.val : 0);
            });

        const auto solution = sum_of_unmarked * drawn_n;
