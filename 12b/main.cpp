...
bool has_path_small_double(const PathTree          &tree,
                           const std::vector<Cave> &caves,
                           NodeIdx                  end_of_path) {
  NodeIdx current_node = end_of_path;
  while (true) {
    if (current_node == 0) break;
    const CaveIdx cave_idx   = tree.nodes[current_node].cave_idx;
    const NodeIdx parent_idx = tree.nodes[current_node].parent;
    if (!caves[cave_idx].is_big && is_cave_in_path(tree, parent_idx, cave_idx)) return true;
...
bool try_direction(PathTree                &tree,
  ...
  // check that this cave is not start (we are not allowed to go back to start)
  ...
  // if this cave is small, it must either be the first time we visit it, or the first time we visit
  // a small cave twice
  if (!target.is_big && is_cave_in_path(tree, current_idx, target_idx) &&
      has_path_small_double(tree, caves, current_idx)) {
    return false;
  }

  // add node to children, and recurse
  tree.nodes.emplace_back(target_idx, std::vector<NodeIdx>{}, current_idx);
  NodeIdx new_node_idx = tree.nodes.size() - 1;
  tree.nodes[current_idx].children.push_back(new_node_idx);

  bool path_reached_end = false;
  for (CaveIdx dir : target.connected_caves) {
    const bool dir_reaches_end = try_direction(tree, caves, new_node_idx, dir);
...
int main() {
  ...
  const auto solution = std::count_if(tree.nodes.begin(),
                                      tree.nodes.end(),
                                      [end_idx](const Node &n) { return (n.cave_idx == end_idx); });
