...
void expand_cavern(Cavern & cavern, int n = 5) {
  cavern.width *= static_cast<size_t>(n);
  cavern.height *= static_cast<size_t>(n);

  const auto orig = cavern.risk_levels;

  cavern.risk_levels.clear();
  cavern.risk_levels.reserve(cavern.height);

  for(int y = 0; y < n; y++) {
    for(const auto & orig_row : orig) {
      cavern.risk_levels.emplace_back();
      auto & new_row = cavern.risk_levels.back();
      new_row.reserve(cavern.width);

      for(int x = 0; x < n; x++) {
        std::transform(orig_row.begin(), orig_row.end(), std::back_inserter(new_row), [&](int l) {
          auto new_value = l + x + y;
...
int main() {
  auto cavern = get_game_input_from_file("input.txt");

  std::cout << cavern << std::endl;

  expand_cavern(cavern);
