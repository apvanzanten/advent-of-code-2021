...
int get_min_x_velocity(const Target &t) {
...
int get_max_x_velocity(const Target &t) { return t.right_bottom.x; }
int get_min_y_velocity(const Target &t) { return t.right_bottom.y; }
int get_max_y_velocity(const Target &t) { return -(t.right_bottom.y) - 1; }
...
int main() {
  ...
  for (const auto &target : targets) {
    const Velocity min_velocity{get_min_x_velocity(target), get_min_y_velocity(target)};
    const Velocity max_velocity{get_max_x_velocity(target), get_max_y_velocity(target)};

    std::cout << "trying velocities between " << min_velocity << " and " << max_velocity
              << std::endl;

    std::vector<Velocity> valid_velocities{};

    for (int x = min_velocity.x; x <= max_velocity.x; x++) {
      for (int y = min_velocity.y; y <= max_velocity.y; y++) {
        const auto velocity   = Velocity{x, y};
        const auto trajectory = Probe::launch(velocity).shoot_for_target(target);

        if (target.contains(trajectory.back())) {
          valid_velocities.push_back(velocity);

          std::cout << "valid velocity: " << velocity << std::endl;
          // print_trajectory_and_target(trajectory, target);
        }
      }
    }
    std::cout << "total valid: " << valid_velocities.size() << std::endl;
  }
}
